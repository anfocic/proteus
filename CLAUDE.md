...
**Session token budget.** `trimToBudget({ messages, maxTokens, tokenize, preserveLast?, perMessageOverhead? })` (`src/agent/budget.ts`) is a pure helper for trimming a `Message[]` history to a token budget. Walks newest → oldest, drops the oldest first, always keeps at least `preserveLast` (default 1). `tokenize` is required — model-specific tokenizers exist; shipping a default `chars/4` heuristic would silently mislead. Counts string content directly; for assistant content blocks, sums text + reasoning + `JSON.stringify` of tool_use inputs. Per-message overhead defaults to 3 tokens (intrebit's value). Caller decides when to apply (load time, before each LLM call, or both).
...
- `classifyIntent` (`src/agent/router.ts`) — single LLM call, low temperature, returns `Classification { intents: Intent[], mode: "single" | "chain" | "parallel", reasoning?, usage }`. The router prompt asks for JSON only; output is parsed by `tryParseJSON` (`src/agent/json-repair.ts`) which strips markdown fences and repairs unclosed strings/brackets/trailing commas — necessary for local-model hosts (lmstudio etc.) that frequently truncate. Reads `text` blocks, falls back to `reasoning` blocks if text is empty (handles thinking models like GLM). Unknown intent names are dropped; if none remain or parse fails entirely, falls back to `[registered[0]]` with `mode: "single"`. `maxIntents` (default 3) caps fan-out. ADR 0012.
- `orchestrate` (`src/agent/orchestrate.ts`) — `classifyIntent` → switch on `cls.mode` → dispatch. Single mode runs `cls.intents[0]`'s specialist via `runSpecialist`. Chain mode iterates `cls.intents` sequentially, augmenting each step's user message with the prior step's `finalText` wrapped in `<previous_step_output>...</previous_step_output>` (truncated to `chainContextChars`, default 2000); `OrchestrateResult.steps?: ChainStep[]` is populated; `routedTo` and `finalText` come from the last step; failures surface as `ChainDispatchError` with completed steps preserved. Custom formatter via `chainContextFormatter`. Parallel mode runs `Promise.allSettled` over matched specialists against the original message; fulfilled `finalText`s join with `\n\n---\n\n` (override via `parallelAggregator`); rejections preserved in `steps` but dropped from the joined text; `messages: []` + `iterations: 0` (not meaningful for fan-out); `routedTo` is comma-joined intent names. `evaluate` is rejected for chain and parallel modes (semantics deferred). `"pending"` confirm decisions in parallel mode escalate to a top-level throw — parallel is intended for read-only fan-out. `streamOrchestrate` throws for chain and parallel (deferred). Caller passes separate `routerModel` and `specialistModel`. ADR 0013.
...
**Evaluator gate.** `OrchestrateOpts.evaluate?: EvaluatorFn` runs after the specialist returns; on `ok: false` the specialist is re-run with `verdict.feedback` (or a default reconsider prompt) appended as a user message. Bounded by `maxEvaluatorAttempts` (default 2 = 1 retry; 1 = evaluate-only telemetry mode). At cap with `ok: false` the last result is returned without throwing — caller detects via `evaluatorAttempts === maxEvaluatorAttempts`. Result gains `evaluatorAttempts` + `evaluatorUsage`; `specialistUsage` becomes the sum across attempts; `usage` total = router + specialist + evaluator. Pure callback, no built-in evaluator — consumer writes the LLM call. `streamOrchestrate` and `resumeOrchestrate` do NOT support `evaluate` (streaming would duplicate output; resume would diverge from fresh-run behaviour). Setting `evaluate` on the streaming path throws early. `ChatHandlerConfig` threads it on the buffered handler only. ADR 0009.
