...
- `orchestrate` (`src/agent/orchestrate.ts`) — `classifyIntent` → switch on `cls.mode` → dispatch. Single mode runs `cls.intents[0]`'s specialist via `runSpecialist`. Chain and parallel modes are not yet implemented (throw on the dispatch); they land in subsequent ADRs. Caller passes separate `routerModel` and `specialistModel` so cheap routing + capable specialist is one config away.
